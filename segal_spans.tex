...
\end{lemma}
\begin{proof}
  %We first that the map $i$ is a full subcategory inclusion. The $n$-simplices of $A \backslash X$ are maps of marked simplicial sets $\tilde{\sigma}\colon A \square \Delta^{n} \to X$. A map of underlying simplicial sets respects the markings if and only if for each $(a, i) \in A_{1} \times (\Delta^{n})_{0}$ with $a$ marked, $\tilde{\sigma}(a, i)$ is marked in $X$. This is equivalent to demanding that $\sigma|_{\Delta^{\{i\}}}$ belong to $A \backslash X$.
...
\begin{definition}
  ...
  \begin{itemize}
    ...
    \item Denote by $\Sigma_{n}$ the space
      ...
      \end{equation*}
      That is, $\Sigma_{n}$ is the union of $\Delta^{\{0, 1\}}$ with the spine of $d_{1}\Delta^{n}$. We will call $\Sigma_{n}$ the \defn{left spine} of $\Delta^{n}$.
  \end{itemize}
\end{definition}
