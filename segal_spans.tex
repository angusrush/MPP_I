...
\section{Marked bisimplicial sets}
\label{sec:marked_bisimplicial_sets}

We need a basic theory of marked bisimplicial sets. This is a very ugly formulation of something that's probably equivalent to something pretty. However, in the case we're interested, it's good enough.

\begin{definition}
  A \defn{marked bisimplicial set} is a bisimplicial set $X$ together with a marking on $i_{1}^{*}X$. We interpret this as a subset of $X_{10}$.
\end{definition}

\begin{definition}
  For a marked simplicial set $A$ and a marked bisimplicial set $B$, define a marking on the bisimplicial set $A \square B$ as follows: a simplex $(a, b) \in A_{1} \times B_{0}$ is marked if and only if $a$ is marked.
\end{definition}

\begin{definition}
  Let $A$ denote a marked simplicial set, $B$ an unmarked simplicial set, and $X$ a marked bisimplicial set.
  \begin{itemize}
    \item Define an unmarked simplicial set $(A \backslash X)'$ by the universal property
      \begin{equation*}
        \Hom_{\SSet}(B, (A \backslash X)) = \Hom_{\SSSet^{+}}(A \square B, X).
      \end{equation*}

    \item Define a marking on the simplicial set $X / B$ as follows: a 1-simplex $\Delta^{1} \to X / B$ is marked if and only if the corresponding map $\Delta^{1} \square B \to X$ of unmarked bisimplicial sets comes from a map of marked bisimplicial sets $(\Delta^{1})^{\sharp} \square B \to X$.
  \end{itemize}
\end{definition}

I still have to check with a fine-toothed comb that we have adjunctions
\begin{equation*}
  A \square - : \SSet \longleftrightarrow \SSSet : A \backslash -
\end{equation*}
and
\begin{equation*}
  - \square B : \SSet^{+} \longleftrightarrow \SSSet^{+} : - / B,
\end{equation*}
but I've sketched a proof.
...
\begin{corollary}
  Let $\category{C} \to \category{D}$ be a morphism of marked $\infty$-categories each of whose markings is closed under composition. Suppose that the underlying unmarked morphism $f\colon \Gamma(\category{C}) \to \Gamma(\category{D})$ is a Reedy fibration. Let $u\colon A \to B$ be a morphism of marked simplicial sets whose underlying morphism is a monomorphism. Then the map $\langle u \backslash f \rangle$ is a Kan fibration.
\end{corollary}
...
\begin{definition}
  ...
  \begin{itemize}
    ...
    \item Denote by $\Sigma_{n}$ the space
      \begin{equation*}
        \Sigma_{n} = \Delta^{\{0, 1\}} \amalg_{\Delta^{\{0\}}} I_{\{0, \hat{1}, 2, \ldots, n\}}.
      \end{equation*}
...
Note that $\Sigma_{2} \simeq \Lambda^{2}_{0}$.
...
\begin{lemma}
  Let $f\colon X \to Y$ be a Reedy fibration between Segal spaces, and let $e \in X_{10}$ be a morphism with the property that the square
  \begin{equation*}
    \begin{tikzcd}
      \Delta^{2} \backslash X \times_{\Delta^{\{0, 1\}} \backslash X} \{e\}
      \arrow[r]
    ...
    \end{tikzcd}
  \end{equation*}
  is homotopy pullback. Then the square
  \begin{equation*}
    \begin{tikzcd}
...
For any simplicial subset $A \subseteq \Delta^{n}$, denote the marking where the only marked nondegenerate edge is $\Delta^{\{0, 1\}}$ by $A^{\mathcal{L}}$.
...
\begin{theorem}
  Let $f\colon X \to Y$ be a Reedy fibration between complete Segal spaces such that over each morphism $\bar{e} \in Y_{10}$ there exists a cocartesian lift $e \in X_{10}$, i.e.\ a cocartesian morphism $e \in X_{10}$ such that $fe = \bar{e}$. Then the map $i_{1}^{*}X \to i_{1}^{*}Y$ is a cocartesian fibration of quasicategories.
\end{theorem}
\begin{proof}
  It suffices to show that if a morphism in $X_{10}$ is $f$-cocartesian, then it is $i^{*}_{1}f$-cocartesian. To see this, note that
\end{proof}

\section{Basics of fibrations complete Segal spaces}

\begin{itemize}
  \item The inclusion of Kan complexes into quasicategories has right adjoint $(-)^{\simeq}$, which sends a simplicial set to the largest kan complex it contains. sets.
...
Our proof of \hyperref[thm:span_of_bicartesian_fibration_is_bicartesian_fibration]{Theorem~\ref*{thm:span_of_bicartesian_fibration_is_bicartesian_fibration}} will involve working with with bicartesian fibrations. For this reason, it will be helpful to have results about simplicial sets with two markings, one of which controls the cocartesian structure and one of which controls the cartesian structure. This section essentially verifies that some key results about marked simplicial sets which can be found in \cite[Sec.\ 3.1]{highertopostheory} hold in the bimarked case. The only results we will make use of are \hyperref[proposition:bimarked_anodyne_homotopy_pullback]{Proposition~\ref*{proposition:bimarked_anodyne_homotopy_pullback}}, \hyperref[eg:bimarking_on_asd_delta1]{Example~\ref*{eg:bimarking_on_asd_delta1}}, and \hyperref[eg:bimarking_on_asd_delta2]{Example~\ref*{eg:bimarking_on_asd_delta2}}.
