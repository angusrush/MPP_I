...
\begin{example}
  ...
  \begin{equation*}
    B \backslash X^{\sharp} \cong \mathring{B} \backslash X.
  \end{equation*}
\end{example}
...
\begin{appendix}
  ...
  If both $\category{E}_{1}$ and $\category{E}_{2}$ are finitely complete and $\category{E}_{3}$ is finitely cocomplete, then from a map $u\colon A \to B$ in $\category{E}_{1}$ and a map $f\colon X \to Y$ in $\category{E}_{3}$, we get a map
  \begin{equation*}
    X / B 
  \end{equation*}
  ...
  \begin{proposition}
    The box functor is \emph{divisible on the left and on the right.} This means that for each simplicial set $A$, there are adjunctions
    \begin{equation*}
      A \square -\colon \SSet \leftrightarrow \SSSet : A \backslash -,
    \end{equation*}
    ...
    \begin{equation*}
      - \square A\colon \SSet \leftrightarrow \SSSet : - \backslash A.
    \end{equation*}
  \end{proposition}
  \begin{proof}
    We prove the first; the second is identical. We do this by explicitly exhibiting a bijection
    \begin{equation*}
      \begin{tikzcd}
    ...
    Now define a map $\ev\colon A \square (A \backslash X) \to X$ level-wise as follows: we take $(a, \sigma) \in A_{m} \times (A \backslash X)_{n}$ to
    \begin{equation*}
    ...
    The maps $\Phi$ and $\Psi$ are mutually inverse, and provide the necessary bijection.
  \end{proof}
