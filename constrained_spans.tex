...
For example, for $n = 4$, such an $n$-simplex consists of a diagram of the form
\begin{equation*}
  \begin{tikzcd}[column sep=tiny, row sep=small]
    \\
    &&&& X_{04}
...
Fix some adequate triple $\triple{C}$. We will call a functor $\sd(\Delta^{n}) \to \category{C}$ \emph{ambigressive Cartesian} if each square in $\sd(\Delta^{n}) = N(\Sigma_{n})$ of the form
\begin{equation*}
...
To that end, for an adequate triple $\triple{C}$ and any simplicial set $A$, we make the following defintion.
\begin{definition}
  \label{def:acart}
  We define a simplicial set $\Map^{\mathrm{aCart}}(\sd(A), \category{C})^{\simeq}$ to be the full simplicial subset
  \begin{equation*}
...
\begin{equation*}
  ...
  \end{tikzcd}.
\end{equation*}
Even if $g$ is not invertible, we can think of our span as representing formally a map `$f \circ g^{-1}\colon X \to X'$', which may not exist in $\category{C}$.
...
\begin{example}
  ...
  For any quasicategory $\category{C}$, we can define a triple $(\category{C}, \category{C}\downdag = \category{C}, \category{C}\updag = \category{C}^{\simeq})$. These choices of ingressive and egressive morphisms correspond to spans of the form
  \begin{equation*}
    \begin{tikzcd}
      & Y
      \arrow[dl, swap, "\simeq"]
...
\begin{equation*}
  ...
  \end{tikzcd},
\end{equation*}
and the map $\sd(\Delta^{n}) \to \Delta^{n}$ `collapses' the backward-facing legs of the spans. The general case is hopefully clear.
...
\end{equation*}
which is a level-wise Kan weak equivalence, and hence a weak equivalence in the complete Segal spaces model structure.\footnote{For a brief review of the model structures in question, see \hyperref[sss:a_brief_list_of_model_structures]{Appendix~\ref*{sss:a_brief_list_of_model_structures}}. For more information, see \cite{qcats_vs_segal_spaces}, especially the discussion following Thm.\ 4.1.} Here $\Gamma(\category{C})$ is the complete Segal space defined in \cite{qcats_vs_segal_spaces} in the discussion preceding Thm.\ 4.11. In \cite{qcats_vs_segal_spaces} it is also discussed that the functor $( - )/\Delta^{0}\colon \SSSet^{\mathrm{CSS}} \to \SSet^{\mathrm{Joyal}}$ is the right Quillen functor in a Quillen equivalence between the Complete Segal Space model structure and the Joyal model structure. Thus, because $\Gamma(\category{C})$ and $\SPAN^{\simeq}(\category{C})$ are complete Segal spaces, the restricion of $i$ to first rows $i / \Delta^{0}$ is a weak categorical equivalence, which is what we wanted to show.
\end{example}

\hyperref[eg:spans_with_equivalences_on_one_leg]{Example~\ref*{eg:spans_with_equivalences_on_one_leg}} allows us to find a copy of $\category{C}$ in a category of spans. Similarly, if we look at spans whose forward-facing legs are equivalences, we find a copy of $\category{C}\op$.
...
\begin{theorem}
  ...
  Then $\pi$ is an inner fibration, and if a morphism in $\Span\triple{C}$ is of the form
  \begin{equation*}
    \begin{tikzcd}
      & y
