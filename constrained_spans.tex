...
\begin{example}
  For any quasicategory, we can define a triple $(\category{C}, \category{C}\downdag = \category{C}, \category{C}\updag = \category{C}^{\simeq})$. The corresponding spans are of the form
  \begin{equation*}
    \begin{tikzcd}
      & y
      \arrow[dl, swap, "\simeq"]
    ...
    \end{tikzcd},
  \end{equation*}
  where $Y \to X$ is an equivalence in $\category{C}$.

  The resulting quasicategory of spans is categorically equivalent to the category $\category{C}$ from which we started. To see this, note that for each $n \geq 0$ there is a retraction of posets
  \begin{equation*}
    [n] \hookrightarrow \Sigma_{n} \twoheadrightarrow [n],
  \end{equation*}
  where the first map takes $i \mapsto (i, n)$, and the second takes $(i, j) \mapsto i$. Under the nerve, this corresponds to a retraction of simplicial sets
  \begin{equation*}
  ...
  \begin{equation*}
    ...
    \end{tikzcd}.
  \end{equation*}
  Note that the image of the inclusion $i$

  It is not hard to convince oneself that a map $\sd(\Delta^{n}) \to \category{C}$ is ambigressive cartesian if and only if it is a right Kan extension of its restriction along $\Delta^{n} \hookrightarrow \sd(\Delta^{n})$. In the case $n = 2$, the inclusion $[2] \hookrightarrow \Sigma_{2}$ is of the form
  \begin{equation*}
    ...
    \end{tikzcd}.
  \end{equation*}
  The right Kan extension of a diagram $\Delta^{2} \to \category{C}$ 

  The inclusions $\Delta^{n} \to \sd(\Delta^{n})$ does not yield a map of cosimplicial objects $\Delta^{\bullet} \to \sd(\Delta^{\bullet})$, but the projections $\sd(\Delta^{n}) \to \Delta^{n}$ do, giving us a map of cosimplicial objects $\sd(\Delta^{\bullet}) \to \Delta^{\bullet}$. 

  There is an equivalence of Segal spaces
  \begin{equation*}
    \Gamma(\category{C}) \to \SPAN(\category{C}, \category{C}, \category{C}^{\simeq}),
  \end{equation*}
  where $\Gamma(\category{C})$ is the Segal space defined level-wise by
  \begin{equation*}
    \Gamma(\category{C})_{n} = \Fun(\Delta^{n}, \category{C}).
  \end{equation*}
  This sends a morphism $f\colon x \to y$ in $\category{C}$ to the span
  \begin{equation*}
    \begin{tikzcd}
      & x
      \arrow[dl, equals]
...
This theorem originally appears in \cite{spectralmackeyfunctors1} (although, as noted above, the assumptions are slightly different). However, the proof there is combinatorial in nature. Our proof uses the material developed in \hyperref[sec:cocartesian_fibrations_between_complete_segal_spaces]{Section~\ref*{sec:cocartesian_fibrations_between_complete_segal_spaces}}, leveraging the fact that $\infty$-categories of spans have natural incarnations as (complete) Segal spaces.

Before we get to the meat of the proof, however, we have to prove some preliminary results.

\subsection{Housekeeping}

In this section we recall some basic facts we will need in the course of our proof of the main theorem. 
